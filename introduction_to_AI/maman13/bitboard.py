...
class PlayerBitBoard(BitBoardCalculator):
    def __init__(self, player: ColorDiskPlayer, board_size: int = 8, bitboard: int = 0):
    ...
    def initial(self):
        ...
        if self.player == ColorDiskPlayer.RED:
            mid = self.board_size // 2
            initial_cells = [(mid - 1, mid - 1), (mid, mid)]

        if self.player == ColorDiskPlayer.WHITE:
            mid = self.board_size // 2
...
if __name__ == '__main__':
    import sys
    bitboard = PlayerBitBoard(player=ColorDiskPlayer.RED, board_size=8)
