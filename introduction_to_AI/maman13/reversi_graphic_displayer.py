...
class ReversiGraphicDisplayer:
    ...
    def _color_to_face(self, player: ColorDiskPlayer) -> str:
        return "red" if player == ColorDiskPlayer.RED else "white"
    ...
    def add_disc(self, player: ColorDiskPlayer, r: int, c: int) -> None:
        disc = Circle(
            (c + 0.5, r + 0.5),
            0.38,
    ...
    def render_state(self, state) -> None:
        ...
        while bb:
            ...
            self.add_disc(ColorDiskPlayer.RED, r, c)
            bb ^= lsb
        ...
        while bb:
            lsb = bb & -bb
            r, c = self._bit_to_rc(lsb)
            self.add_disc(ColorDiskPlayer.WHITE, r, c)
            bb ^= lsb

    # ------- overlays -------

    def update_score(self, state) -> None:
        red = state.score(ColorDiskPlayer.RED)
        white = state.score(ColorDiskPlayer.WHITE)
