...
class ReversiGameRunner(BitBoardCalculator):
    ...
    def __init__(
        ...
        self.problem = ReversiGameProblem()

        self.agents: Dict[ColorDiskPlayer, MinMaxAgent] = {
            ColorDiskPlayer.RED: red_agent,
        ...
        print(self.current_state.white_bitboard.bitboard)

        print("after")
        print(self.snapshot())

        # for debug, save players moves
        self.moves_history: List[Tuple[ColorDiskPlayer, ReversiMove]] = []
    ...
    def bit2move(self, bit: Optional[int]) -> ReversiMove:
        if bit is None:
            return ReversiMove.pass_move()

        row, column = self.bit2cell(bit)
        return ReversiMove(row, column)

    def legal_moves_bits(self) -> List[Optional[int]]:
        return self.current_state.legal_moves()

    def legal_moves_as_moves(self) -> List[ReversiMove]:
        return [self.bit2move(bit) for bit in self.legal_moves_bits()]

    def current_agent(self) -> MinMaxAgent:
        return self.agents[self.current_state.player_turn]
    ...
    def next_step(self) -> None:
        ...
        self.current_turn += 1
        player = self.current_state.player_turn
        agent = self.current_agent()

        move: ReversiMove = agent.choose_move(self.current_state)
    ...
    def play(self, max_turns: int = 10_000, stop_at: int = None) -> ReversiGameState:
        ...
        while not self.current_state.is_terminal() and self.current_turn < max_turns:
    ...
    def check_cell(self, cell: Tuple[int, int]) -> Optional[ColorDiskPlayer]:
        ...
        if self.current_state.red_bitboard.is_bit_on(bit):
            return ColorDiskPlayer.RED

        if self.current_state.white_bitboard.is_bit_on(bit):
    ...
    def get_current_scores(self):
        red_score = self.current_state.score(ColorDiskPlayer.RED)
        white_score = self.current_state.score(ColorDiskPlayer.WHITE)
    ...
    def print_current_scores(self, is_final=False):
        ...
        if not is_final:
            print(f"\nresult - RED={red_score}, WHITE={white_score}, "
                  f"TOTAL={get_occupied_bitmask(self.current_state.red_bitboard, self.current_state.white_bitboard).bit_count()}")
