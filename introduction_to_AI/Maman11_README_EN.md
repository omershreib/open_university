...
This *maman* required to compare 3 algorithmic solutions attempt to solve this Tiles game:
- BFS (Un-informed deterministic algorithm)
...
- cost-optimal
- requires a high memory usage because it need to save all nodes in memory.
...
1. if h(n) is *consistent*, then h(n) is also *admissible* (not vice-versa).
...
(in a relaxed case scenario) towards its goal (x, y) position. This derives more simple idea: 
...
- moving in/out to/from its goal row/column change WrongRowCol score by $\pm 1$
...
*LinearConflict* (LC) attempts to improve Manhattan Distance (MD) by an additional *penalty* addon, donated by what called: the number of linear conflicts in a current state.
...
were tested by 4 different start board configuration ordered from the easiest to the hardest one,
in terms of the shortest path available to solve the 8-Tiles game problem when these board are
set to be the initial board states:
...
[[1, 2, 0],         [[1, 4, 0],         [[1, 6, 3],         [[6, 4, 8],
 [3, 4, 5],         [5, 8, 2],          [0, 4, 5],          [7, 5, 1],
 [6, 7, 8]]         [3, 6, 7]]          [7, 2, 8]]          [2, 3, 0]] 
 
 (easiest)                                                  (hardest)
```

![shortest_path_comparison_per_board.png](maman11%2Fcharts%2Fshortest_path_comparison_per_board.png)


As depicted from this comparison figure below, in terms of number of nodes expansions, 
BFS is considered to be the worst. Second-worst is Misplaced, and LinearConflict is the most dominant
heuristic. The performance of my WrongRowCol is somewhere between Misplaced and ManhattanDistance.


![expanded_nodes_comparison_per_board.png](maman11%2Fcharts%2Fexpanded_nodes_comparison_per_board.png)
...
Only `BFS`, `WrongRowsColumns`, and `LinearConflict` will be formally explained as part
...
- WrongRowsColumns 'wrong_row_col' 
